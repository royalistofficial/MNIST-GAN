...
def compute_real_width(image):
    ...
    _max = np.max(temp)
    _min = np.min(temp)
    return 2*(temp - _min) / (abs(_max - _min) + 1e-8)
...
def evaluate(netD, test_loader, netG=None):
    adversarial_loss = nn.BCEWithLogitsLoss()
    auxiliary_loss = nn.CrossEntropyLoss()
    width_loss = nn.MSELoss()
    ...
    print(f"--- Реальные изображения ---")
    print(f"  Ошибка дискриминации: {avg_real_adv:.4f}")
    print(f"  Ошибка классификации: {avg_real_class:.4f}")
    print(f"  Ошибка ширины:        {avg_real_width:.4f}")

    if netG is not None:
        print(f"--- Фейковые изображения ---")
        print(f"  Ошибка дискриминации: {avg_fake_adv:.4f}")
        print(f"  Ошибка классификации: {avg_fake_class:.4f}")
        print(f"  Ошибка ширины:        {avg_fake_width:.4f}")

    return {
        "real": (avg_real_adv, avg_real_class, avg_real_width),
        "fake": (avg_fake_adv, avg_fake_class, avg_fake_width)
    }
...
def plot_losses(history, epoch):
    ...
    plt.subplot(2, 2, 1)
    plt.plot(history["g_loss"], label="Ошибка генератора (train)")
    plt.plot(history["d_loss"], label="Ошибка дискриминатора (train)")
    plt.xlabel("Эпоха")
    plt.ylabel("Ошибка")
    plt.legend()
    plt.title("Обучение (train)")

    plt.subplot(2, 2, 2)
    plt.plot(history["test_real_adv"], label="D (реальные)")
    plt.plot(history["test_fake_adv"], label="D (фейковые)")
    plt.xlabel("Эпоха")
    plt.ylabel("Ошибка")
    plt.legend()
    plt.title("Тест (test)")
    ...
    plt.subplot(2, 2, 3)
    plt.plot(history["test_real_class"], label="Class (реальные)")
    plt.plot(history["test_fake_class"], label="Class (фейковые)")
    plt.xlabel("Эпоха")
    plt.ylabel("Ошибка")
    plt.legend()
    plt.title("Тест (test)")
    ...
    plt.subplot(2, 2, 4)
    plt.plot(history["test_real_width"], label="Width (реальные)")
    plt.plot(history["test_fake_width"], label="Width (фейковые)")
    plt.xlabel("Эпоха")
    plt.ylabel("Ошибка")
    plt.legend()
    plt.title("Тест (test)")
    ...
    plt.close()
    print("Графики ошибок сохранены!")
...
def train():
    ...
    optimizerD = optim.AdamW(netD.parameters(), lr=LR, betas=(BETA1, BETA2), weight_decay=WEIGHT_DECAY)
    optimizerG = optim.AdamW(netG.parameters(), lr=LR, betas=(BETA1, BETA2), weight_decay=WEIGHT_DECAY)

    fixed_noise = torch.randn(10 * 10, NZ, device=DEVICE)
    fixed_labels = torch.arange(0, 10, dtype=torch.long, device=DEVICE).repeat(10)
    fixed_widths = torch.linspace(0.1, 3, steps=10, device=DEVICE).repeat_interleave(10)
    ...
    history = {
        "d_loss": [], "g_loss": [],
        "test_real_adv": [], "test_real_class": [], "test_real_width": [],
        "test_fake_adv": [], "test_fake_class": [], "test_fake_width": []
    }

    # results = evaluate(netD, test_loader, netG)
    # history["test_real_adv"].append(results["real"][0])
    # history["test_real_class"].append(results["real"][1])
    # history["test_real_width"].append(results["real"][2])
    # history["test_fake_adv"].append(results["fake"][0])
    # history["test_fake_class"].append(results["fake"][1])
    # history["test_fake_width"].append(results["fake"][2])
